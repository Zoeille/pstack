...
```text
load pstack:poteto-mode users get two notifications after a retry. repro first, then fix and verify.
```
...
```text
load pstack:poteto-mode new task. figure out why the cache entry survives logout. don't change any code yet.
```
...
If you run several agents against one repository, they can fight over the working tree. Use delegate_task to give each agent its own branch and scope:

```text
load pstack:poteto-mode new task. branch off <base>, then port the parser change there.
```

Each task in its own branch means no agent stomps another's files. The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) already works from a branch for code changes, so mostly you only say this when a specific base or location matters.
...
```text
load pstack:poteto-mode im stepping away. keep going until the migration check reports zero old callers. log your decisions.
```
...
**Pitfall:** don't enumerate skills in your prompt ("use pstack:how, then pstack:architect, then pstack:interrogate..."). The playbook already sequences them, and a hand-written sequence usually reorders or drops steps the playbook would have kept. Name a skill only when you want to override a specific choice.

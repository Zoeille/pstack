...
4. [Design the change](./04-design.md). `pstack:architect`, `pstack:swarm`, and `pstack:interrogate` before code locks in a shape.
...
```text
load pstack:poteto-mode the export writes duplicate rows when a retry lands mid-run. repro first, then fix and verify.
```

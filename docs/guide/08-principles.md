...
```text
separate before serializing shared state. give each attempt its own workspace, no locks.
```
...
- [Guard the Context Window](../../skills/principle-guard-the-context-window/SKILL.md) routes bulk reading to delegate_task subagents and keeps findings in the main chat.

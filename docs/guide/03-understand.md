...
```text
load pstack:how do we dedupe notifications? is there an n+1 when we look up subscribers?
```
...
```text
load pstack:why was the retry limit set to five? does the reason still hold?
```
...
```text
load pstack:teach me how this PR changes retries. convince me it fixes the cause and not the symptom.
```
...
```text
load pstack:recall catch me up on the export work from last week
```
...
```text
load pstack:poteto-mode take over this branch. read the decision log, figure out what's done, and continue from there. don't redo finished work.
```
